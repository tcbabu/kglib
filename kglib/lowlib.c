...
#define uiusr_y(y) (float)((y-dc->D_y-dc->v_y1)/dc->u_y+dc->w_y1)
  ...
  void uiMessageString ( DIALOG *D , char *str , int x , int y , int width , int height , int font , int color , int FontSize , int justfic , int bkcolor ) ;
  ...
  int _uiMake_M ( DIM *m ) {
      ...
      if ( m->hide != 1 ) {
          ...
          switch ( m->code ) {
              ...
              uiMessageString ( D , m->msg , ( int ) ( x1 ) , ( int ) ( ( y1 ) ) , ( x2-x1 ) , \
               ( y2-y1 ) , D->gc.MsgFont , D->gc.msg_char , D->gc.GuiFontSize , just , fillclr ) ;
...
//           _ui_diamessage(D,m->x1+D->xo,m->y1+D->yo,m->x2+D->xo,m->y2+D->yo,m->msg);
              ...
              uiMessageString ( D , m->msg , x1 , y1 ,x2-x1 ,y2-y1 , D->gc.MsgFont , D->gc.msg_char , D->gc.GuiFontSize , just , D->gc.msg_fill ) ;
