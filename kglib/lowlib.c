...
//  kgImage(D,img,x1+5,y1,ln,w->width,0.0,1.0);
  ...
  int _uiMake_W ( DIW *w ) {
      ...
      if ( w-> hide != 1 ) {
          ...
          if ( n != 0 ) {n++;lngth = 8;}
          lngth += n*6;
          if ( lngth > ( ( w-> x2-w-> x1 ) *0.8+32 ) ) lngth = 
              ( w-> x2-w-> x1 ) *0.8+32;
          w-> xb = w-> x1+lngth;
          i = 0;
          while ( ( w-> menu [ i ] ) != NULL ) i++;
...
#endif
      xshift = ( y-> x2 -y-> x1-2*y-> offset-y-> w ) - 
          ( y-> lngth+y-> xgap ) *y-> nx;
      xshift = xshift/2;
...
#if 0
          kgRoundedRectangle ( fid , xp , yp , ( float ) width , ( float ) height , D-> gc.ProgBodrColor , 0.25 , 0.5 ) ;
...
//  ln = tx->ln[curbox];
      ...
      switch ( type ) {
          ...
          if ( img == NULL ) {
              ...
              img = kgPressedRectangle ( x2-x1+2 , abs 
                  ( y2-y1 ) , tx-> gc.txt_fill , 0.5 ) ;
...
//  uidraw_inset(D,X1,D->evgay-1-Y1,X2,D->evgay-1-Y2);
      ...
      else _dvrect_fill_transparent ( wc , ( x1 ) , ( y1 ) , ( x2 ) , 
          ( y2 ) , D-> gc.fill_clr , D-> transparency ) ;
      uiRest_clip_limits ( wc ) ;
      return 1;
  }
...
//      if(xoffset==0 ) xoffset=2;
      ...
      if ( y-> bordr == 1 ) {
          _dv_draw_bound ( D , ( br-> x1+xoffset ) , ( br-> y1+xoffset ) , ( br-> x2-xoffset-w ) , 
              ( br-> y2-xoffset ) , D-> gc.twin_bodr ) ;
          _dv_draw_bound ( D , ( br-> x1+xoffset+1 ) , ( br-> y1+xoffset+1 ) , ( br-> x2-xoffset-w-1 ) , 
              ( br-> y2-xoffset-1 ) , D-> gc.twin_bodr ) ;
